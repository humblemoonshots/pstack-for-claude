...
- **Treating `inherit` as a model of its own.** `inherit` means "omit the model field so the subagent runs on the parent chat model." [Setup](./01-setup.md) covers the roles.
