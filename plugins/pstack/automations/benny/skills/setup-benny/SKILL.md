...
Do this before asking for Benny configuration and before invoking the `/schedule` skill.
...
Use only model slugs shown as available in the session's available model list (`opus`, `fable`, `sonnet`, `haiku`, or `inherit`). Do not guess a slug and do not carry over a private default.
...
Never call a direct automation backend service or backend automation tool. Never use a browser URL that carries draft fields. Never build or open an automation protocol deep link. For new automations, the only finish path is the built-in `automate` skill's reviewed Automations editor handoff.
