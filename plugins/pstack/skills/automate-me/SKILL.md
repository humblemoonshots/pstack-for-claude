...
This skill orchestrates three others: an inline mining pass (see step 1), the **Authoring or modifying a skill** playbook (`../poteto-mode/playbooks/authoring-a-skill.md`, authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. They live in `~/.claude/projects/<workspace-slug>/`, where the slug is the workspace path with every character outside [A-Za-z0-9-] turned into `-`; this session's id is in $CLAUDE_CODE_SESSION_ID. Use only that directory. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Author the skill per the **Authoring or modifying a skill** playbook (`../poteto-mode/playbooks/authoring-a-skill.md`). Placement:
...
- Frontmatter formatting: follow the [Claude Code skill frontmatter reference](https://code.claude.com/docs/en/skills#frontmatter-reference). Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
