...
Before spawning investigators, list the MCP servers available in this session. Use the tool list when present; `ToolSearch` and `/mcp` both enumerate connected servers.
...
- `general-purpose` inherits every tool, including MCP servers, which MCP-backed investigators need. Investigators still shouldn't write anything. That's a posture, not a sandbox: say so in the brief.
...
- `general-purpose`, not `Explore`: the synthesizer's quality check spot-verifies citations, which can require MCP access.
