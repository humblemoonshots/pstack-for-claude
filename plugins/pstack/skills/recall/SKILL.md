...
Transcripts live at `~/.claude/projects/<slug>/<session-id>.jsonl`, with subagent transcripts under `<slug>/<parent-session-id>/subagents/agent-<id>.jsonl`, where `<slug>` is the workspace path with every character outside `[A-Za-z0-9-]` turned into `-` (so `/Users/you/proj` becomes `-Users-you-proj`). The active session's id is in `$CLAUDE_CODE_SESSION_ID`. Every line is one chat message.
