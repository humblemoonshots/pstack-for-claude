...
If a phase creates or edits a skill, the phase instructs the implementer to follow the **Authoring or modifying a skill** playbook (`../playbooks/authoring-a-skill.md`).
...
- Browser / Electron / Web UIs: Claude Code's bundled `/run` and `/verify`, driving the page through the Chrome MCP tools.
- CLIs and TUIs: Claude Code's bundled `/run` and `/verify`.
- Either way, a project-local `verify-*` skill from `/create-verification-skill` beats both when one exists.
