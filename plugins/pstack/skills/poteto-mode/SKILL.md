  ...
  UserPromptSubmit:
    - matcher: "*"
      hooks:
        - type: command
          command: |
            printf '%s' '{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"[poteto-mode] New task? Playbook match or rigor needed -> apply poteto-mode. Casual turn or user opts out -> do not."}}'
---
...
- Before commit → the **unslop** skill (`/unslop`) over anything prose-shaped in the diff, then Claude Code's bundled `/simplify` over the code.
- Before review → the **no-comments** skill (`/no-comments`).
- Shipping UI / IDE / CLI → drive the real surface with Claude Code's bundled `/run` and `/verify`, and the Chrome MCP tools for browser UIs. A project-local `verify-*` skill from `/create-verification-skill` beats both; prefer it when one exists. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
