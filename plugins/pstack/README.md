...
**multi-model gives you the best of all worlds.** every frontier model has its strengths and weaknesses. use any model with pstack. in fact, many of my skills use multi-model workflows to take advantage of each model's unique strengths.
...
the marketplace manifest lives at `.claude/.claude-plugin/marketplace.json`, so nothing new lands at the repo root. give the marketplace a repo-specific name; marketplace names are global, so a generic `local` collides the moment you vendor pstack into a second repo.

`marketplace add` records the marketplace in *user* settings with an absolute path, which does not survive a clone. to make the repo self-contained, declare it in the committed `.claude/settings.json` instead and drop the user-level entry:

```json
{
  "extraKnownMarketplaces": {
    "<repo-name>": { "source": { "source": "directory", "path": "${CLAUDE_PROJECT_DIR}/.claude" } }
  },
  "enabledPlugins": { "pstack@<repo-name>": true }
}
```

or load it for one session without installing anything:

```bash
claude --plugin-dir ./.claude/pstack
```
...
[`/poteto-mode`](./skills/poteto-mode/SKILL.md) works extremely well with Claude Code's `/loop` skill. you can make claude code work for many hours without sacrificing rigor.
...
- `/simplify` is a claude code bundled skill; it replaces cursor's `deslop` in this fork.
- `/run` and `/verify` are claude code bundled skills; they cover the runtime verification that cursor's `control-cli` and `control-ui` did.
- skill authoring routes to the `superpowers:writing-skills` skill when you have it installed; otherwise the [authoring a skill playbook](./skills/poteto-mode/playbooks/authoring-a-skill.md) tells you to write the `SKILL.md` directly against the [claude code skills docs](https://code.claude.com/docs/en/skills).
- for pr-status requests, the [babysit playbook](./skills/poteto-mode/playbooks/babysit.md) owns the loop.
...
models are configurable too. type [`/setup-pstack`](./skills/setup-pstack/SKILL.md). it detects the models you have access to and writes `.claude/pstack-models.md`, mapping each role (code, judgment, the review panels) to a model. every skill reads it and falls back to sensible defaults when it is absent, so you override only what you want.
...
to set it up, point claude code at [`FOR_AGENTS.md`](./automations/benny/FOR_AGENTS.md). **note:** benny was written against cursor automations; on claude code the equivalent scheduling layer is [routines](https://code.claude.com/docs/en/routines) (`/schedule`), and the pack's two automation prompts still need porting to it. setup copies the pack into the target repository at `.claude/automations/benny/`, enables pstack there for shared skills, and keeps user configuration outside the copied pack.

## claude code fork notes

upstream pstack targets cursor. this fork changes the following and nothing else:

| upstream (cursor) | this fork (claude code) |
|---|---|
| `.cursor-plugin/plugin.json` | `.claude-plugin/plugin.json`, with the `skills`/`agents` path fields dropped so both directories auto-discover |
| model slugs (`gpt-5.6-sol-max`, `grok-4.6-fast-xhigh`, `claude-fable-5-thinking-max`) | the aliases `opus`, `fable`, `sonnet`, `haiku`, `inherit` |
| `~/.cursor/rules/pstack-models.mdc` with `alwaysApply` | `.claude/pstack-models.md`, read on demand by each skill |
| the `Task` tool, `subagent_type: generalPurpose` | the `Agent` tool, `subagent_type: general-purpose` |
| per-call `readonly: true` | `subagent_type: Explore`, whose tool set excludes edits |
| `environment: "cloud"` / `"local"` | `isolation: "remote"` / omitted |
| `mode: true` + `reminder:` sticky-mode frontmatter | a `UserPromptSubmit` hook in `poteto-mode`'s frontmatter that re-injects the same reminder every turn |
| `agent-transcripts/` | `~/.claude/projects/<workspace-slug>/<session-id>.jsonl` |
| cursor built-in `create-skill` | `superpowers:writing-skills` when installed, else the claude code skills reference |
| `deslop` from `cursor-team-kit` | claude code's bundled `/simplify` |
| `control-ui` / `control-cli` from `cursor-team-kit` | claude code's bundled `/run` and `/verify` |

to pull upstream changes, re-run the sparse checkout of [cursor/plugins](https://github.com/cursor/plugins), diff `pstack/` against this directory, and re-apply the table above to whatever moved. the fork touches prose and the manifest only, so upstream edits usually merge cleanly.

known gaps: reasoning effort is a session setting in claude code, not a per-`Agent`-call one, so the multi-model panels vary by model but not by effort tier; the [benny automation pack](./automations/benny/) still needs its triggers ported to routines.
